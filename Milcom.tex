...
For example, after the Enron dataset was released, researchers were able to provide insight into employee behaviors based on the available data despite the many challenges with that dataset.
...
Based solely on email metadata, a random forest approach modeled behavior patterns and classified employees by job title with $96\%$ accuracy.
...
The study presented in this paper collected an appropriately anonymized email metadata dataset to demonstrate to what extent this metadata could be used to determine this organic organizational chart.
...
Retention of large email archives has become common practice as computer memory technology imporves and becomes increasingly affordable.
...
For example, the work by Gilbert~\cite{gilbert2012phrases} showed that the wording used in an email could be used to make inferences about the corporate hierarchy.
However, email text contains very sensitive infromation and is often unavailable for a general email study.
Therefore, features based on email text are not considered in this work.

Using features extracted from email metadata alone,~\cite{yelupula2008social} was able to cluster levels of management at Enron.
In addition to email traffic statistics, using features such as the presence of different email attachment types and the length of emails has been shown to successfully categorize email behavior~\cite{martin2005analyzing}.
...
In~\cite{tyler2005mail}, a betweenness centrality algorithm was used to determine community structures within an organization.
Other successful metrics were used in~\cite{wilson2009discovery}, which detected the most important email users within a corporate network without using betweenness as a feature.
...
The aim of this paper is to utilize both types of features in order to produce a measureable comparison between institution's organic chart and its official organizational chart.
...
Whereas thehe social features aim to quantify relationships within the community structure of the organization.
...
Utilizing the timestamp informaiton, the total number of emails sent and received after hours and the number of emails sent and received after hours within the center were also used as metrics.
...
Additional features were calculated from the total number of signed or encrypted emails sent and received as well as unique email addresses and subjects with signed or enctypted emails.
...
Additionally, the average neighbor degree of each node was computed as a feature.
...
\cite{page1999pagerank} also to rank webpages for search results.
